namespace Dreamteck.Splines
{
    ...
    public partial class SplineComputer : MonoBehaviour
    {
        ...
        public SplinePoint[] GetPoints(Space getSpace = Space.World)
        {
            ...
            return points;
        } public SplinePoint[] GetPointsLocal(Space getSpace = Space.Local)
        {
            SplinePoint[] points = new SplinePoint[spline.points.Length];
            for (int i = 0; i < points.Length; i++)
            {
        ...
        public List<Vector3> getPointsPositon(){
            List<Vector3> pointPosition=new List<Vector3>();
            SplinePoint[] points=GetPoints();
            for(int i=0;i<points.Length;i++){
                pointPosition.Add(points[i].position);

            }
          return pointPosition;

        }
        public void changePointPos(int index , int zvalue){
            spline.points[index].position.z=zvalue;
             Rebuild();

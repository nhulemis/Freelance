 ...
 * Fixed ParticleSystems using transform scale. Legacy did not support this.
 * Fixed size grow. grow is not linear, we also did not handle min and max times.
 * 
 * v1.3
 * Fixed compilation issues on 2017.1
 * Warning added for Unity version 2018.3 and newer. Legacy ParticleSystems will be removed in 2018.3.
 * 
 * v1.4
...
public class LegacyParticleSystemUpdater : ScriptableWizard
{
    ...
    enum LegacyParticleSystemRenderMode
    {
        Billboard = 0,
        Stretch2D = 1,
        Stretch3D = 3,
   ...
   private ParticleSystem[] components;
    ...
    {
        ScriptableWizard.DisplayWizard<LegacyParticleSystemUpdater>("Upgrade Legacy ParticleSystems v" + kVersion, "Upgrade Selected", "Upgrade Everything");
    }

    void OnWizardUpdate()
    {
#if UNITY_2018_3_OR_NEWER
        helpString = @"This Script adds ParticleSystem and ParticleSystemRenderer Components to all GameObjects that contain Legacy ParticleSystem Components.
        This script is not support in this version, please run this script using a Unity version between 2017.4 and 2018.2.";
#else
        helpString = @"This Script adds ParticleSystem and ParticleSystemRenderer Components to all GameObjects that contain Legacy ParticleSystem Components.
        Legacy ParticleSystem System Components can be deleted, disabled, or preserved for comparison.
        Please run this script using Unity 2017.4 or newer.";
#endif
    }

#if !UNITY_2018_3_OR_NEWER // Legacy ParticleSystems were removed

    // Find selected assets
    void OnWizardCreate()
    {
        components = Selection.GetFiltered<ParticleSystemEmitter>(SelectionMode.Unfiltered);
        prefabs = null;
    ...
    {
        List<ParticleSystemEmitter> results = new List<ParticleSystemEmitter>();
        for (int i = 0; i < EditorSceneManager.sceneCount; i++)
        {
            ...
            {
                ParticleSystemEmitter[] emitters = root.GetComponentsInChildren<ParticleSystemEmitter>(true);
                results.AddRange(emitters);
            }
        }
        components = results.ToArray();

        string[] prefabGUIDs = AssetDatabase.FindAssets("t:ParticleSystemEmitter");
        prefabs = new ParticleSystemEmitter[prefabGUIDs.Length];
        for (int i = 0; i < prefabGUIDs.Length; i++)
        {
            prefabs[i] = AssetDatabase.LoadAssetAtPath<GameObject>(AssetDatabase.GUIDToAssetPath(prefabGUIDs[i])).GetComponent<ParticleSystemEmitter>();
        }
    ...
    void Upgrade(ParticleSystemEmitter emitter)
    {
        // Add component
        ...
        // Shape
        if (emitter is EllipsoidParticleSystemEmitter)
        {
        ...
        ParticleSystemAnimator animator = emitter.GetComponent<ParticleSystemAnimator>();
        if (animator != null)
        {
            // Color over lifetime
        ...
        if (renderer != null)
        {
            ...
            var renderMode = (LegacyParticleSystemRenderMode)serializedObjectRenderer.FindProperty("m_StretchParticleSystems").intValue;
            switch (renderMode)
            {
                case LegacyParticleSystemRenderMode.Billboard:
                    psr.renderMode = ParticleSystemRenderMode.Billboard;
                    break;

                case LegacyParticleSystemRenderMode.Stretch2D:
                case LegacyParticleSystemRenderMode.Stretch3D:
                    psr.renderMode = ParticleSystemRenderMode.Stretch;
                    break;

                case LegacyParticleSystemRenderMode.SortedBillboard:
                    psr.renderMode = ParticleSystemRenderMode.Billboard;
                    psr.sortMode = ParticleSystemSortMode.Distance;
                    break;

                case LegacyParticleSystemRenderMode.BillboardFixedHorizontal:
                    psr.renderMode = ParticleSystemRenderMode.HorizontalBillboard;
                    break;

                case LegacyParticleSystemRenderMode.BillboardFixedVertical:
            ...
            psr.maxParticleSystemSize = serializedObjectRenderer.FindProperty("m_MaxParticleSystemSize").floatValue;
        ...
        {
            Undo.DestroyObjectImmediate(emitter.gameObject.GetComponent<ParticleSystemAnimator>());
            Undo.DestroyObjectImmediate(emitter.gameObject.GetComponent<ParticleSystemRenderer>());
            Undo.DestroyObjectImmediate(emitter);
        }
        else if (cleanupMode == LegacyCleanupMode.DisableLegacyRenderer)
        {
            Undo.RecordObject(emitter.gameObject.GetComponent<ParticleSystemRenderer>(), "Disable Object");
            emitter.gameObject.GetComponent<ParticleSystemRenderer>().enabled = false;
        }
    }
#endif
}

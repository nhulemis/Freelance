...
public class ObstacleSpawner : MonoBehaviour
{
    ...
    {
        GameObject _obj =  obstacles[obstacleId];
        
        if(obstacleId < obstacles.Length-1)
        {
            obstacleId++;
        }
        else
    ...
    {
        GameObject _obj = scoreObjects[scoreObjectsId];

        if (scoreObjectsId < scoreObjects.Length - 1)
        {
            scoreObjectsId++;
        }
        else

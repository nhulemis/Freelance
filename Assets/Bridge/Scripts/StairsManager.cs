...
public class StairsManager : MonoBehaviour
{
 ...
 private void Awake()
    ...
    public void makeNewChild(Vector3 childPos,string color)
    {
        StartCoroutine(delayMakingNewChild( childPos,  color));
    }
    IEnumerator delayMakingNewChild(Vector3 childPos, string color)
    {
    ...
    public Transform getNextTarget(string color,int levelCollecting)
    {
        ...
        List<Transform> matchedStairs = getMatchingColorStairs(color, levelCollecting);
    ...
    public List<Transform> getMatchingColorStairs(string c,int levelC)
    {
    ...
    private void makeEnnemycollectableStairs(string color,int parent)
    {
            ...
            for(int i = 0; i < 6; i++)
            {
                Transform childParent = getChildParent(parent);
                if (childParent != null)
                {
    ...
    private Transform getChildParent(int parentLevel)
    {

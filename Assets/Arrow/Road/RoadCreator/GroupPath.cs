...
{
	public class GroupPath : LevelPart
	{
		...
		[SerializeField]
		[Range(0.0f, 0.5f)]
		private float turnCurvePercent = 0.2f;
		...
		private const float scaleUpSize = 1.5f;
		...
		public void CalculateEvenlySpacedPoints()
		{
			...
			for (int i = 0; i < points.Length - 1; i++)
			{
				...
				rightPoint = Vector3.Lerp(points[i], points[i + 1], turnCurvePercent);
				...
				rightPath.Create(new Vector3[] {
					...
					endPoint ? points[i + 1] : Vector3.Lerp(points[i + 1], points[i], turnCurvePercent)
				},
				SPACING,
				RESOLUTION
				);
